...
def getPlottableReactions( reactionSuite, observable='crossSection' ):
    ...
    '''
    result = []
    for reactionList in reactionSuite.reactions, reactionSuite.sums, reactionSuite.fissionComponents, reactionSuite.productions:
...
def generatePlot( observable, dataSets, xyData=[], xydyData=[], xdxydyData=[], plotStyle={}, suggestTitle='',
                  suggestXLog=False, suggestYLog=False, suggestFrame=None, figsize=(20,10),
...
def getEXFORSets( sym, A, reaction = None, quantity = "SIG", nox4evals = True, nox4legend = False,
    ...
    print fudge.core.utilities.brb.banner( "Preparing EXFOR data for " + sym+'-'+A +'(' +reaction.upper()+')'+', '+ quantity )
...
def getXdXYdYDataSets( xdxydyData, plotStyle ):
    ...
    xdxydyDataSets=[]
    for k in xdxydyData:
        thisSetStyle=plotstyles.getThisSetStyle(plotStyle,'xydxdyCurve',k)
...
def makeCrossSectionPlot( gndMap={}, xyData={}, xydyData={}, xdxydyData={}, c4File=None,
    ...
    for endf in gndMap:
        ...
        try:
            ...
            if thisReaction is None:
                    ...
                    if nounc: rawEndfUnc.append( None )
                        ...
                        if covariance is None:
                            ...
                            try: rawEndfUnc.append( covariance.getUncertaintyVector(relative = False) )
                                ...
                                print "WARNING: Adding uncertainty from %s for MT %i failed with error \"%s\"" % ( endf, mt, str(err) )
                                rawEndfUnc.append( None )
                ...
                except TypeError, err: print "WARNING: Adding data from %s for MT %i failed with error \"%s\"" % ( endf, mt, str(err) )

            # and plot all the rest, they don't get covariances...
            if showparts:
                if isinstance( thisReaction, fudge.gnd.sums.crossSectionSum ) \
                        or mt in range( 851, 871 )\
                        or mt in [1,3,4,16,18,27,101,103,104,105,106,107]:
                    if not isinstance( thisReaction, fudge.gnd.sums.crossSectionSum ):
                        raise NotImplementedError("FIXME: ENDF sum rules not coded as cross section sums!")
                    for summand in thisReaction.summands:
                        # I shouldn't have to do this, but apparently I need to
    ...
    exforData = []
    if ( not nox4 ) and ( not reactionName is None ):
...
def makeAngDistLegendreMomentPlot( gndMap={}, xyData={}, xydyData={}, xdxydyData={},
    ...
    def get_LegendreMoments(theAngDist):
        ...
        Lmax = len(theAngDist[-1][-1].coefficients)-1
        theLegMoments={L:[] for L in range(Lmax+1)}
        lastE = -1
        for region in theAngDist:
            for eLegPair in region:
                E = eLegPair.value
                if E <= lastE: continue
                else: lastE = E
                for L,coeff in enumerate(eLegPair.coefficients):
        ...
        # Convert them to XYs
        return { L:XYs.XYs(data=theLegMoments[L], axes=coeffAxes) for L in range(Lmax+1) }
