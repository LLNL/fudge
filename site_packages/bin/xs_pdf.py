...
try:
    ...
    def jit(func, signature=None, nopython=False, nogil=False, cache=False, forceobj=False, locals={}): return func
...
#    if args.verbose: print('binnedPDF before loading:', _binnedPDF[gndsRxnMap[MT]])
    ...
    for iep, ep in enumerate(_junkXS):
        ...
        for jXS, eta in enumerate(indicatorFuncs):
            if jXS==iXS: eta[iep]=[ep[0],1.0]
            else: eta[iep]=[ep[0],0.0]
...
def getPDFFromPURROutput( purrFile, energyBin, verbose=False ):
    if energyBin is None: energyBin=0
    lines = file(purrFile).readlines()
    line=''
    nTemps=0
    nSigma0=0
    nLadders=0
    ...
    while lines and not line.startswith(" ******************"):
        ...
        if line.startswith('e='):
            ...
            if NE == energyBin:
                ...
                while lines and not line.startswith(" probability table"): line=lines.pop(0)
                ...
                while lines and not line=='\n':
                    ...
                    if line.startswith(' tmin'):
                        ...
                        while line.startswith('    ') or line.startswith(' tmax'):
                            if line.startswith(' tmax'): xsBins+=[float(x) for x in line.split()[2:]]
                            else: xsBins+=[float(x) for x in line.split()]
                            line = lines.pop(0)
                        NXS=len(xsBins)-1
                        print("NXS =", NXS)
                        print('xsBins:', xsBins)
                    ...
                    elif line.startswith(' tot') or line.startswith(' els') or line.startswith(' fis') or line.startswith(' cap'):
                        rxn=line.split()[0]
                        xs[rxn]=[]
                    ...
                    # nothing... keep going
                    else: line = lines.pop(0)

            NE += 1
    if verbose: print('probs:',probs)
    if verbose: print(xs.keys())
    print( "final NE =",NE)
    print('eBins:',eBins)
...
def __avexs_from_xspdf( xsPdf, NE, eBinCenters, eBinWidths, NXS, xsBinCenters, xsBinWidths ):
    ...
    for iE in np.arange(NE):
        ...
        if __EdEXSdXS[iE, 3] > 0.0:
            for iXS in np.arange(NXS):
                __EdEXSdXS[iE, 4] += xsPdf[iE, iXS] * pow((xsBinCenters[iXS] - __EdEXSdXS[iE, 2]) / __EdEXSdXS[iE, 3], 3.0) * xsBinWidths[iXS]
                __EdEXSdXS[iE, 5] += xsPdf[iE, iXS] * pow((xsBinCenters[iXS] - __EdEXSdXS[iE, 2]) / __EdEXSdXS[iE, 3], 4.0) * xsBinWidths[iXS]

        if __EdEXSdXS[iE, 2]==0.0: continue
        ...
        __EdEXSdXS[iE, 9]=math.exp(4.*sig*sig)+2.0*math.exp(3.*sig*sig)+3.*math.exp(2.*sig*sig)-3.0
...
def function_to_XYs( func, fpars,
               ...
               domainUnit='eV', domainName='energy_in', rangeUnit='b', rangeName='crossSection', accuracy=upperEps ):
    '''
...
def fit_XY_points( _xdata, _ydata, _yerror, form='const', convertToXYs=True ):
    ...
    dpars.reverse()
    if convertToXYs: return function_to_XYs(lambda x, y: f(x, *(popt.tolist())), [])  # awkward conversion if I ever saw one...
    return popt, pcov
...
def process_args():
    ...
    parser.add_argument('--pdfFormat', type=str, default='default', choices=['PURR','default'], help="The format of file with the URR pdf (if given)")
    ...
    # Energy grid
    parser.add_argument('--Emin', type=float, default=defaultEmin, help="Emin in MeV (default=%s MeV)"%str(defaultEmin))
    parser.add_argument('--Emax', type=float, default=defaultEmax, help="Emax in MeV (default=%s MeV)"%str(defaultEmax))
    ...
    parser.add_argument('--readSetModifications', type=str, default=None, help='JSON file with potential file modifications')
    parser.add_argument('--editSets', default=False, action="store_true", help="Edit the sets so that they are between Emin & Emax, etc")
    ...
    parser.add_argument('--thicken', default=0, type=int,
                        help="Thicken (reconstructed) cross sections by this number of points between existing points (default = 0, i.e. don't thicken" )

    # Treatment of PDFs read from a file
    parser.add_argument('--noRenormalize', default=False, action="store_true", help="Do not renormalize the PDFs read in from a file")
    ...
    parser.add_argument('--aveXSMode', default="pdf", choices=['quad', 'linear', 'const', 'pdf', 'endf', 'mughabghab'], help="Mode to compute the average")
    parser.add_argument('--getXSPDF', default=False, action="store_true", help="Compute the cross section probability distribution")
    parser.add_argument('--getXSCorr', default=False, action="store_true", help="Compute the cross section energy-energy correlation")
    ...
    parser.add_argument('--plotXSCorr', default=False, action="store_true", help="Plot the cross section energy-energy correlation")
    parser.add_argument('--printXSCorr', default=False, action="store_true", help="Print the cross section energy-energy correlation")
...
if __name__ == "__main__":
    ...
    if not args.ignoreData or args.source == 'data':
        ...
        if args.readSetModifications is not None:
            if args.verbose: print("# Reading local modifications from %s"%args.readSetModifications)
        ...
        if args.editSets:
            ...
            for x4Set in x4Sets:
                ...
                if x4Set.legend in localMods and localMods[x4Set.legend]['exclude']:
                    if args.verbose: print("# Excluding %s"%x4Set.legend)
                    continue
                ...
                if min(x4Set.x) > args.Emax or max(x4Set.x) < args.Emin: continue
                ...
                if localMods[x4Set.legend]["E scale"] != 1.0 or localMods[x4Set.legend]["E shift (MeV)"] != 0.0:
                    x4Set.x=[x*localMods[x4Set.legend]["E scale"]+localMods[x4Set.legend]["E shift (MeV)"] for x in x4Set.x]
            ...
            for x4Set in x4Sets:
                ...
                if True and aveEndfXSOverDomain is not None:
                    ...
                    for ipt in range(len(x4Set.x)):
                        if x4Set.x[ipt] > args.Emax or x4Set.x[ipt] < args.Emin: continue
                        if x4Set.yerr[ipt] <= 0.0: continue
                    ...
                    if len(xx)>1:
                        ...
                        if abs(math.log(xsNormFactor))>=1.:
                            print( "# Set %s requires an unreasonable renormalization (%s) the 'exclude' flag is being set to True" % (x4Set.legend, str(xsNormFactor)))
                            exclude = True
                    else:
                        print("# Set %s has too few usuable points (%i) in the region of interest so the 'exclude' flag is being set to True"%(x4Set.legend,len(xx)))
                        exclude=True
    ...
    if args.pdf is not None:
        ...
        if args.pdfFormat == 'default':
            ...
            if    args.plotPDFSlice is not None:
                ...
                args.plotPDFSlice = 0  # a kludge because I can't read the full PURR table all at once, so I only have one bin
            elif args.printPDFSlice is not None:
                sliceBin=args.printPDFSlice
                args.printPDFSlice=0 # a kludge because I can't read the full PURR table all at once, so I only have one bin
            urrPDF=getPDFFromPURROutput(args.pdf, sliceBin)
        else:
            raise ValueError("Don't know PURR format '%s'"%args.pdfFormat)
        if args.noRenormalize: pass
        else: urrPDF.normalize()
    ...
    if args.getXSCorr:
        ...
        for x4Set in x4Sets:
            ...
            for i in range(len(x4Set.x)):
                if stopSet: break
                if x4Set.x[i] > args.Emax or x4Set.x[i] < args.Emin: continue
                if x4Set.yerr[i] <= 0.0: continue
                for j in range(i+1,len(x4Set.x)):

                    # Basic point pair quality control
                    if x4Set.x[j] > args.Emax or x4Set.x[j] < args.Emin: continue
                    if x4Set.yerr[j] <= 0.0: continue
                    if x4Set.x[i]==x4Set.x[j]: continue
            ...
            epsilonList = expRTemp[x4Set.legend].keys()
            epsilonList.sort()

            # Integrate to get the correlator
            minNpts=10
            for epsilonKey in epsilonList:
                if False: Npts=1./binSizeRatio  # Simpson's rule suggests this
                else:     Npts=len(expRTemp[x4Set.legend][epsilonKey]) # Monte-Carlo approach suggests this
                if Npts < minNpts: continue
            ...
            if len(expR[x4Set.legend])>2:
                ...
                if True: # Latex summary table of data sets used
                    ...
                    Npts=len(x4Set.x)
                    NptsInRange=len([x for x in x4Set.x if (x<=args.Emax and x>=args.Emin)])
                ...
                if saveXSCorr:
                    ...
                    for x in expR[x4Set.legend]:
                        setOut.write('  '.join(map(str,x))+'\n')
            else:
                print('    Did not write R(eps) for ', x4Set.legend, 'it has too few points ('+str(len(expR[x4Set.legend]))+')')
                pass
...
#                if "Weston" in k: continue
                if len(expR[k])<=2: continue # not worth using in fit
            ...
            for k in expRChunk:
                ...
                for x in expRChunk[k]:
                    if x[0]<epsilonMin or x[0]>epsilonMax: continue # off the grid, we can't use the point
                    xdataR.append(x[0])
                    ydataR.append(x[1])
                    yerrR.append(x[3])
                if len(xdataR)==0: continue
                results = fit_series(xdataR, ydataR, yerrR, seriesInstance=Rfunc, priorModel=priorModel, priorCov=priorCov)
