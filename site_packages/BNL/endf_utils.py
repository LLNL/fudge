...
def getEvaluationMTs( reactionSuite, mtFilter = None ):
    ...
    if mtFilter is None: return [ r.ENDF_MT for r in reactionList ]
...
def getPointwiseCrossSection( reac ):       # FIXME remove? logic is broken + appears to be unused
    if   'linear'            in reac.crossSection.forms:
        ...
        return reac.crossSection['resonancesWithBackground'].background.toPointwise_withLinearXYs( accuracy = 1e-3, upperEps = 1e-8 )
    return None


def getUncertainty( theCovariance, theData ):
    """ extract absolute uncertainty vector (ie, in units of barn) from covariance """
    from fudge.gnds.covariances.base import covarianceMatrix
    from fudge.gnds.covariances.mixed import mixedForm
    theUncert = None
