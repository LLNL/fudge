...
def OBJECT2HTML(d, nametag="<strong>%s: </strong>", itemtag='<li>%s</li>', valuetag="%s", blocktag=('<ul>', '</ul>')):
    def islistlike(x): return isinstance(x, list) or isinstance(x, tuple) or isinstance(x, set)
    def isdictlike(x): return isinstance(x, dict) or isinstance(x, collections.OrderedDict)
    def dorecursion(x): return islistlike(x) or isdictlike(x)
    r=[]
    if isdictlike(d):
        r.append(blocktag[0])
...
__symbolDict = dict( [ [ x[8:], x[0:7] ] for x in '''&#0913; Alpha
&#0914; Beta
&#0915; Gamma
&#0916; Delta
&#0917; Epsilon
...
def SYMBOL( key ): return __symbolDict[ key ]
