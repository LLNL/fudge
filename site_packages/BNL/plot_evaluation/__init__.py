...
def getPlottableReactions( reactionSuite, observable='crossSection' ):
    ...
    '''
    result = []
    for reactionList in reactionSuite.reactions, \
                        reactionSuite.sums.crossSections, \
                        reactionSuite.sums.multiplicities,\
                        reactionSuite.fissionComponents, \
                        reactionSuite.productions:
...
def getSuggestTitle( target, projectile, reaction, mt ):
    ...
    if reaction.lower() == 'inel' or mt >=600:
        ...
        if   mt in range(50,91):    reactionString = "n["+str( mt%50 )+"]"
        elif mt == 91:              reactionString = "n[c]"
        elif mt in range(600,649):  reactionString = "p["+str( (mt-550)%50 )+"]"
        elif mt == 649:             reactionString = "p[c]"
        elif mt in range(650,691):  reactionString = "d["+str( (mt-600)%50 )+"]"
        elif mt == 691:             reactionString = "d[c]"
        elif mt in range(700,749):  reactionString = "t["+str( (mt-650)%50 )+"]"
        elif mt == 749:             reactionString = "t[c]"
        elif mt in range(750,791):  reactionString = "3He["+str( (mt-700)%50 )+"]"
        elif mt == 791:             reactionString = "3He[c]"
        elif mt in range(800,849):  reactionString = "a["+str( (mt-750)%50 )+"]"
        elif mt == 849:             reactionString = "a[c]"
        elif mt in range(850,891):  reactionString = "2n["+str( (mt-800)%50 )+"]"
        elif mt == 891:             reactionString = "2n[c]"
        else:                       reactionString = 'inel'

    elif mt in range(500,573):
        # Atomic reaction data
        if mt == 500:           reactionString = "Total charged particle stopping power"
        elif mt==501:           reactionString = "Total photon interaction"
        elif mt == 502:         reactionString = "Photon coherent scattering"
        elif mt == 504:         reactionString = "Photon incoherent scattering"
        elif mt in [505,506]:   reactionString = "Photon scattering factor"
        elif mt == 515:         reactionString = "Pair production, electron field"
        elif mt == 516:         reactionString = "Total pair production"
        elif mt == 517:         reactionString = "Pair production, nuclear field"
        elif mt == 522:         reactionString = "Photo-electric absorption"
        elif mt == 523:         reactionString = "Photo-excitation"
        elif mt == 526:         reactionString = "Electro-atomic scattering"
        elif mt == 527:         reactionString = "Bremstrahlung"
        elif mt == 528:         reactionString = "Electro-atomic excitation"
        elif mt == 533:         reactionString = "Atomic relaxation"
        else:
...
def generatePlot( observable, dataSets, xyData=[], xydyData=[], xdxydyData=[], plotStyle={}, suggestTitle='',
    ...
    if thisPlotStyle["xAxis"]["log"] == None:
        if suggestXLog is not None: thisPlotStyle["xAxis"]["log"] = suggestXLog
    if thisPlotStyle["yAxis"]["log"] == None:
        if suggestYLog is not None: thisPlotStyle["yAxis"]["log"] = suggestYLog
...
def getEXFORSets( sym, A, reaction = None, quantity = "SIG", nox4evals = True, nox4legend = False,
                  forceLegend = False, plotSyle={}, verbose=True ):
    exforData = []
    try:
    ...
    if verbose: print(fudge.core.utilities.brb.banner( "Preparing EXFOR data for " + sym+'-'+str(A) +'(' +reaction.upper()+')'+', '+ quantity ))
    if verbose: print('Retrieving entries:'+str( subents.keys() ))
    suppressEXFORLegend = ( nox4legend or len( subents.keys() ) > 20 ) and not forceLegend
    if verbose: print('Search parameters:', sym+'-'+str(A), reaction, quantity)
    if verbose: print('Found following (sub)entries:')
    for e in subents:
        if nox4evals and e.startswith( 'V' ): continue
        if verbose: print('    Entry:',e)
        try:
        ...
        for d in ds:
            ...
            if len( dat ) > 0:
                ...
                exforData.append( DataSet2d( data = dat, uncertainty = unc, xUnit=ds[d].units[0], yUnit=barnsConverter( ds[d].units[1] ), legend = theLegend, lineStyle = ' ', symbol = plotstyles.getPlotSymbol( i ), color=plotstyles.getPlotColor(theLegend,False) ) )
                i += 1
    if verbose: print()
    return exforData
...
def getC4DataSets( c4File, mt, mf, target, projectile, plotSyle={} ):
    try:
        from site_packages.BNL import c4
    except ImportError:
    ...
    for p in flist:
        ...
        if lastSet != thisSet:
            if lastSet != ( None, None ):
                ...
                if mf == 3:   c4Data.append( DataSet2d( data = dat, uncertainty = unc, xUnit='eV', yUnit='b',           legend = theLegend, lineStyle = ' ', symbol = plotstyles.getPlotSymbol( i ), color=plotstyles.getPlotColor(theLegend,False) ) )
                elif mf == 4: c4Data.append( DataSet3d( data = dat, uncertainty = unc, xUnit='eV', yUnit='', zUnit='b', legend = theLegend, lineStyle = ' ', symbol = plotstyles.getPlotSymbol( i ), color=plotstyles.getPlotColor(theLegend,False) ) )
                dat = []
                unc = []
            i+=1
            lastSet = thisSet
...
def makeCrossSectionPlot( gndsMap={}, xyData=[], xydyData=[], xdxydyData=[], c4File=None,
    ...
    for endf in gndsMap:
        ...
        try:
            ...
            except KeyError: thisReaction = None
            ...
            if thisReaction is None:
                ...
                try:
                    ...
                    if nounc: rawEndfUnc.append( None )
                    else:
                        ...
                        if covariance is None:
                            ...
                            try: rawEndfUnc.append( covariance.getUncertaintyVector(relative = False) )
                ...
                except TypeError as err: print("WARNING: When adding data to plot from %s for MT %i failed with error TypeError(\"%s\")" % ( endf, mt, str(err) ))
    ...
    if endfData + exforData + c4Data != [] and xyData != [] and xydyData != [] and xdxydyData != []:
        if logX is not None: suggestXLog = logX
        else: suggestXLog = mt in [ 1, 2, 18, 102 ] + range(501, 574)
        if logY is not None: suggestYLog = logY
        else: suggestYLog = mt in [ 1, 2, 18, 102 ] + range(501, 574)
...
def makeEnergyTransferPlot( gndsMap={}, xyData={}, xydyData={}, xdxydyData={},
    ...
    for endf in gndsMap:
        ...
        # Extract the cross section data from the endf file
        dist = reactionList[0].outputChannel.getProductWithName(product).distributions.energyTransfer.getNativeData().toPointwise_withLinearXYs( accuracy = 1e-3, upperEps = 1e-8 )
...
def makeFormFactorPlot( gndsMap={}, xyData={}, xydyData={}, xdxydyData={},
    ...
    if endfData != [] and xyData != [] and xydyData != [] and xdxydyData != []:
        ...
        generatePlot( observable = 'formFactor',
                      ...
                      xdxydyData = xdxydyData,
                      plotStyle = plotStyle,
                      suggestTitle = projectile + '+'+ target.capitalize() + ', Photon (in)coherent scattering form factors (MT=502,504)',
                      suggestXLog = len( set(mtList).intersection( range(501, 574) ) ) != 0,
                      suggestYLog = len( set(mtList).intersection( range(501, 574) ) ) != 0,
                      figsize=figsize,
                      useBokeh=useBokeh )
...
def makeMultiplicityPlot( gndsMap={}, xyData={}, xydyData={}, xdxydyData={},
                          ...
                          logY=False,
                          figsize=(20, 10),
                          outFile=None, plotStyle={}, useBokeh=False ):
    ...
    for endf in gndsMap:
        ...
        for p in thisChannel.getProductsWithName(product):
            if thisChannel.isFission():
                if p.attributes['emissionMode'] == 'prompt':label = 'prompt'
                else:
        ...
        if thisChannel.isFission(): totalLegend+=' (total nubar)'
        endfData.append(
            DataSet2d(
                totalMult,
                uncertainty=None,
        ...
        if showparts:
            for p in products:
                ...
                if thisChannel.isFission():
                    if p =='prompt' and not showPrompt: continue
                    if p !='prompt' and not showDelayed: continue
                thisLegend+=' (%s)'%p
                endfData.append(
                    DataSet2d(
                        products[p],
...
def makeEnergyDepositPlot( gndsMap={}, xyData={}, xydyData={}, xdxydyData={},
    ...
    for endf in gndsMap:
        ...
        expectedTotalEnergy = XYs.XYs1d( axes_=Q.axes, data=[ [ x[0],x[1]+x[0] ] for x in Q.copyDataToXYs() ], accuracy=1e-8 )
...
#        Q = reaction.outputChannel.Q.toPointwise_withLinearXYs( accuracy = 1e-3, upperEps = 1e-8 )
        ...
        expectedTotalMom = XYs.XYs1d( axes_=totalMomDep.axes, data=[ [ x[0], math.sqrt(2.0*x[0]*projectileMass) ] for x in totalMomDep.copyDataToXYs() ], accuracy=1e-8 )
...
def makeAngDistMubarPlot( gndsMap={}, xyData={}, xydyData={}, xdxydyData={},
    ...
    # Declare the main reaction type
    try: reactionName = getEXFORRxn( mt )
    except KeyError:
    ...
    for endf in gndsMap:
        ...
        if thisReaction is not None:
            ...
            legend = product+' given as '+str(angDistEvaluated)+" in "+str(angDistEvaluated.productFrame)+' frame ('+endf+')'
            #print angDistPointwise, [ [ E, angDistPointwise.averageMu(E) ] for E in angDistPointwise.getEnergyArray() ]
            endfData.append(
                                DataSet2d(
    ...
    if referenceFrame is None:
        if suggestFrame is None: suggestFrame = 'lab'
    else: suggestFrame = referenceFrame
...
def makeAngDistLegendreMomentPlot( gndsMap={}, xyData={}, xydyData={}, xdxydyData={},
    ...
    def get_LegendreMoments(theAngDist):
        ...
        if isinstance(theAngDist,regions.regions2d): myRegions=theAngDist.regions
        else: myRegions=[theAngDist]
        ...
        for L in theLegMoments:
            ...
            for pair in theLegMoments[L]:
                if len(newList) == 0 or pair[0] > newList[-1][0]: newList.append(pair)
    ...
    for endf in gndsMap:
        ...
        if thisReaction is not None:
            ...
            legend = 'L=%i Legendre moment for %s given in %s frame (%s)' % (L, product, angDistData.ancestor.productFrame, endf)
            print('   ',legend)
