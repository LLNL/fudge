...
static PyObject *crossSectionAdjustForHeatedTarget_py( PyObject *self, PyObject *args, PyObject *keywords ) {
    ...
    for( i = 0; i < n_pairs; i++ ) {
        ...
        xy = PyNumber_Float( PyList_GetItem( E_cs, 0 ) );
        if( xy == NULL ) {
            free( E_cs_in );
            Py_DECREF( iterator );
            Py_DECREF( E_cs );
        ...
        xy = PyNumber_Float( PyList_GetItem( E_cs, 1 ) );
        if( xy == NULL ) {
            free( E_cs_in );
            Py_DECREF( iterator );
            Py_DECREF( E_cs );
...
static char const doc[] = "A module that contains the a function to heat nuclear reaction cross sections.";

#if PY_MAJOR_VERSION < 3

    PyMODINIT_FUNC initcrossSectionAdjustForHeatedTarget( void ) {

        Py_InitModule3( "crossSectionAdjustForHeatedTarget", crossSectionAdjustForHeatedTargetMethods, doc );

    }

#else

    static struct PyModuleDef crossSectionAdjustForHeatedTarget_CModule = {
        PyModuleDef_HEAD_INIT,
        "crossSectionAdjustForHeatedTarget",        /* name of module */
        doc,                                        /* module documentation, may be NULL */
        -1,                                         /* size of per-interpreter state of the module, or -1 if the module keeps state in global variables. */
        crossSectionAdjustForHeatedTargetMethods
    };

    PyMODINIT_FUNC PyInit_crossSectionAdjustForHeatedTarget( void ) {

        return( PyModule_Create( &crossSectionAdjustForHeatedTarget_CModule ) );
    }
