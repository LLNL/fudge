    ...
    for subshell in range(NSS):
        ...
        for i1 in range(NTR):
            ...
            if SUBK == 0:
                ...
                elementID = '%s{%s,%s}' % (
                elementSymbol, MT_AtomicConfigurations[534 + SUBJ - 1], MT_AtomicConfigurations[534 + SUBK - 1])
        ...
        if NTR > 0:
            if abs(1 - probabilitySum) > 1e-4:
                warningList.append('decay probabilities sum to %.6f not 1.' % probabilitySum)

    if offset != len(MF28):
        raise Exception('Not allow lines of MF 28 data processed: offset = %d, len( MF28 ) = %d' % (offset, len(MF28)))
    info.logs.write('\n')
