...
def addDecayMode(info, decayMode, decayParticleIndex, parentZA, RFS, lastDecayMode):
    ...
    if residialProductZA == 1:
        ...
        residialProductID = chemicalElementMiscPoPsModule.idFromZAndA(residialProductZA // 1000,
                                                                      residialProductZA % 1000)
...
def addDiscreteSpectra(decayModes, RTYP_key, decayParticleIndex, discreteSpectra):
    ...
    if 'RICC' in discreteSpectra:
        addInternalConversionCoefficients(discrete, discreteSpectra['RICC'], discreteSpectra['dRICC'],
                                          spectrumModule.Shell.total)
    if 'RICK' in discreteSpectra:
        addInternalConversionCoefficients(discrete, discreteSpectra['RICK'], discreteSpectra['dRICK'],
                                          spectrumModule.Shell.KShell)
    if 'RICL' in discreteSpectra:
        addInternalConversionCoefficients(discrete, discreteSpectra['RICL'], discreteSpectra['dRICL'],
                                          spectrumModule.Shell.LShell)
...
def addContinuumSpectrum(decayModes, RTYP_key, decayParticleIndex, regions, covariance):
    ...
    decayParticleLabel = STYPProduct[decayParticleIndex]
    if RTYP_key not in decayModes: raise KeyError(
        "RTYP %s not found is list of decay modes, RTYP's in this evaluation are %s" % (
        RTYP_key, str(decayModes.keys())))
    spectra = decayModes[RTYP_key].spectra
    if decayParticleLabel not in spectra:
        spectrum = spectrumModule.Spectrum(decayParticleLabel, decayParticleID)
        spectra.add(spectrum)
...
def ITYPE_4(MTDatas, info, verbose=0):
    ...
    parentAtom = toGNDSMisc.getPoPsParticle(info, info.targetZA, name=None, levelIndex=info.levelIndex,
                                            level=info.level,
    ...
    if 457 in MTDatas:
        ...
        ZA, AWR, LIS, LISO, NST, NSP = endfFileToGNDSMisc.sixFunkyFloatStringsToIntsAndFloats(
            MT457MF8Data[dataIndex], intIndices=[2, 3, 4, 5])
        info.ZA_massLineInfo.add(ZA, AWR, 457, 8, AWR_lineNumber, LIS=LIS)
        ...
        if len([aveDecayEnergy for aveDecayEnergy in aveDecayEnergies if (aveDecayEnergy != 0)]) > 0:
            aveDecayEnergies = addAverageDecayEnergyIfPresent(
                decayData, aveDecayEnergies, averageEnergyModule.LightParticles)
            aveDecayEnergies = addAverageDecayEnergyIfPresent(
                decayData, aveDecayEnergies, averageEnergyModule.ElectroMagneticRadiation)
            aveDecayEnergies = addAverageDecayEnergyIfPresent(
                decayData, aveDecayEnergies, averageEnergyModule.HeavyParticles)
            aveDecayEnergies = addAverageDecayEnergyIfPresent(
            ...
            aveDecayEnergies = addAverageDecayEnergyIfPresent(
                decayData, aveDecayEnergies, averageEnergyModule.ConversionElectron)
            aveDecayEnergies = addAverageDecayEnergyIfPresent(
                decayData, aveDecayEnergies, averageEnergyModule.Gamma)
            aveDecayEnergies = addAverageDecayEnergyIfPresent(
                decayData, aveDecayEnergies, averageEnergyModule.XRay)
            aveDecayEnergies = addAverageDecayEnergyIfPresent(
                decayData, aveDecayEnergies, averageEnergyModule.InternalBremsstrahlung)
            aveDecayEnergies = addAverageDecayEnergyIfPresent(
            ...
            aveDecayEnergies = addAverageDecayEnergyIfPresent(
                decayData, aveDecayEnergies, averageEnergyModule.SpontaneousFission)
            aveDecayEnergies = addAverageDecayEnergyIfPresent(
                decayData, aveDecayEnergies, averageEnergyModule.FissionNeutrons)
            aveDecayEnergies = addAverageDecayEnergyIfPresent(
                decayData, aveDecayEnergies, averageEnergyModule.Proton)
            aveDecayEnergies = addAverageDecayEnergyIfPresent(
                decayData, aveDecayEnergies, averageEnergyModule.Neutrino)

        SPI, PAR, dum, dum, NDK6, NDK = endfFileToGNDSMisc.sixFunkyFloatStringsToIntsAndFloats(
        ...
        if NST == 1:  # Nucleus is stable, nothing to do
            ...
            for i1 in range(NSP):  # Iterate over spectra
                zero, STYP, LCON, LCOV, six, NER = endfFileToGNDSMisc.sixFunkyFloatStringsToIntsAndFloats(
                    MT457MF8Data[dataIndex], intIndices=[2, 3, 4, 5])
                dataIndex += printInfo(verbose, dataIndex, MT457MF8Data)
                ...
                FD, dFD, ERave, dERave, FC, dFC = endfFileToGNDSMisc.sixFunkyFloatStringsToFloats(MT457MF8Data[
                                                                                                      dataIndex])  # FD: discrete normalization, FC: continuum norm., ERave: average decay energy
                dataIndex += printInfo(verbose, dataIndex, MT457MF8Data)

                if LCON != 1:  # Discrete spectra given.
                    for i2 in range(NER):
                        ER, dER, dum, dum, NT, dum = endfFileToGNDSMisc.sixFunkyFloatStringsToIntsAndFloats(
                            MT457MF8Data[dataIndex], intIndices=[2, 3, 4, 5])
                        dataIndex += printInfo(verbose, dataIndex, MT457MF8Data)
                        ...
                        if LCON != 1:
                            ...
                            initialDecay, otherDecays, RTYP_key = getRTYP(MT457MF8Data[dataIndex])
                            discreteSpectra['RTYP_plus'] = endfFileToGNDSMisc.sixFunkyFloatStringsToFloats(
                                MT457MF8Data[dataIndex])
                            dataIndex += printInfo(verbose, dataIndex, MT457MF8Data)
                            if verbose > 3: print('STYP, RTYP = ', STYP, RTYP_key)
                            if NT > 6:
                                RICC, dRICC, RICK, dRICK, RICL, dRICL = endfFileToGNDSMisc.sixFunkyFloatStringsToFloats(
                                    MT457MF8Data[dataIndex])
                                dataIndex += printInfo(verbose, dataIndex, MT457MF8Data)
