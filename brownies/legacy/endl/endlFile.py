...
import os
import time
from brownies.legacy.endl import bdfls, endlmisc, endlIClasses, endl2
    ...
    def check( self, targetParameters, dQ_MeV = 10e-3, dThreshold_MeV = 10e-3, allowZeroE = False, xCloseEps = None, normTolerance = 1e-5,
        ...
        for l in self.levels :
            ...
            if( ( targetParameters['T'] != l.Temperature ) or ( targetParameters['ELevel'] != l.ELevel ) or ( targetParameters['halflife'] != l.Halflife ) ) :
                ErrMsgs.append(
                    endlmisc.endlCheckerObject(suffix = suffix, data = l, message ='target parameters differ'))
            elif( X1 is not None ) :
            ...
            if( abs( l.getQ( ) - Q ) > dQ_MeV ) :
                ErrMsgs.append(
                    endlmisc.endlCheckerObject(suffix = suffix, data = l, message ='bad Q: should be %.4e' % Q))
            if( ( self.C != 6 ) and( hasattr( l, "getThresholdsForChecker" ) ) ) :
                ...
                for t in thresholds :   # Currently, thresholds is the first min( n, 2 ) Energy points in the data where n is the length of the data.
                    if( checkQThreshold ) :
                        if( t + _dThreshold_MeV < QThreshold ) : ErrMsgs.append(
                            endlmisc.endlCheckerObject(suffix = suffix, data = l, \
                                                       message = 'bad QThreshold: should be %.8e at index = %d instead of %.8e' % ( QThreshold, iT, t )))
                    else :
                        if( t + _dThreshold_MeV < Threshold ) : ErrMsgs.append(
                            endlmisc.endlCheckerObject(suffix = suffix, data = l, \
...
#
    ...
    def ls( self ) :
        ...
        print( self.name )
        s = ""
        for i in range( len( self.levels ) ) : s = s + ( " levels[%d]  %s" % ( i, repr(self.levels[i]) ) )
        print( s )
