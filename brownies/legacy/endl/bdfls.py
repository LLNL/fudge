        ...
        if( type( za ) == ( type( '' ) ) ) :
            if( za[:2] == 'za' ) :
                ...
                try :
                    ...
                    raise Exception( 'Could not handle za = %s.' % repr(za) )
        else :
            raise Exception( 'za = "%s" cannot be converted into an integer' % repr(za) )
    return( iza )
...
class bdfls :
    ...
    def addZAHalflife( self, za, halflife, warning = 1 ) :
        ...
        for h in self.h :
            if ( h.za == za ) :
                if ( warning ) :
                    fudgemisc.printWarning( "changing halflife for za = %d from %s to %s" % ( za, repr(h.halflife), repr(halflife) ) )
                h.halflife = halflife
                return
            elif ( h.za > za ) :
    ...
    def addZAMass( self, za, mass, warning = 1 ) :
        ...
        for m in self.m :
            if ( m.za == za ) :
                if ( warning ) :
                    fudgemisc.printWarning( "changing mass for za = %d from %s to %s" % ( za, repr(m.mass), repr(mass) ) )
                m.mass = mass
                return
            elif ( m.za > za ) :
    ...
    def save( self, fileName = None ) :
        ...
        s = repr(self.g)
        for g in self.g : f.write( repr(g) )
        f.write( bdfls_EOD )
        for fl in self.f : f.write( repr(fl) )
        f.write( bdfls_EOD )
        ...
        for c in self.c : f.write( repr(c) )
        f.write( bdfls_EOD )
        for t in self.t : f.write( repr(t) )
        f.write( bdfls_EOD )
        f.write( "%4d    subshell designators                                                   *\n" % self.nSubshells )
        for s in self.s : f.write( repr(s) )
        f.write( bdfls_EOD )
        f.close( )
    ...
    def setZAHalflife( self, za, halflife ) :
        ...
        for h in self.h :
            if ( h.za == za ) :
                ...
                raise Exception( "\nError in bdfls.setZAHalflife: no such ZA = %s" % repr(za) )
    ...
    def setZAMass( self, za, mass ) :
        ...
        for m in self.m :
            if ( m.za == za ) :
                ...
                raise Exception( "\nError in bdfls.setZAMass: no such ZA = %s" % repr(za) )
