...
def incidentParticleTags( yi ) :
    ...
    for y in yis :
        if( yi in y ) : return( y )
    raise Exception( "\nError from incidentParticleTags: invalid incident particle tag = %s" % repr(yi) )
...
def headerString2FunkyDouble( s, i, callingRoutine, newMethod = True ) :
    ...
    if( newMethod ) :
        fd = s[i:i+11].strip( )
        n11 = len( fd )             # n11 was 11 but some EPICS files to not contain 11 digits on the last value of a line.
        for i in range( n11 ) :
            if( fd[i] != ' ' ) : break
        if( ( i < n11 ) and ( fd[i] in '-+' ) ) : i += 1
        for i in range( i, n11 ) :
...
After comments are removed, the strings can only contain one numbers per line 
    ...
    if ( i0 != 0 ) :
        raise Exception( "\nError in endlmisc.translate1dStringData: Invalid i0 = %s value" % ( repr(i0) ) )
    d1 = []
    n = None
    for l in ls :
...
y-component is column i1 (i0 and i1 can only be 0 or 1)."""
    ...
    i.sort( )
    if ( i[0] != 0 ) or ( i[1] != 1 ) :
        raise Exception( "\nError in endlmisc.translate2dStringData: Invalid i0 = %s and/or i1 = %s value(s)" % ( repr(i0), repr(i1) ) )
    ConvertDs = 0
...
the z-component is column i2 (i0, i1 and i2 can only be 0, 1 or 2)."""
    ...
    if ( i[0] != 0 ) or ( i[1] != 1 ) or ( i[2] != 2 ) :
        raise Exception( "\nError in endlmisc.translate3dData: Invalid i0 = %s, i1 = %s and/or i2 = %s value(s)" % ( repr(i0), repr(i1), repr(i2) ) )
    x0 = None
    d3 = []
    for d in data :
        x = d[i0]
...
is column i1 and the z-component is column i2 (i0, i1 and i2 can only be 0, 1 or 2)."""
    ...
    i.sort( )
    if ( i[0] != 0 ) or ( i[1] != 1 ) or ( i[2] != 2 ) :
        raise Exception( "\nError in endlmisc.translate3dStringData: Invalid i0 = %s, i1 = %s and/or i2 = %s value(s)" % ( repr(i0), repr(i1), repr(i2) ) )
    x0 = None
    d3 = []
    n = None
    for l in ls :
...
def string3dData( d, i0 = 0, i1 = 1, i2 = 2, fmt0 = None, fmt1 = None, fmt2 = None ) :
    ...
    from brownies.legacy.endl import endl3dmathmisc
    i = [ i0, i1, i2 ]
    i.sort( )
    if ( i[0] != 0 ) or ( i[1] != 1 ) or ( i[2] != 2 ) :
        raise Exception( "\nError in endlmisc.string3dData: Invalid i0 = %s, i1 = %s and/or i2 = %s value(s)" % ( repr(i0), repr(i1), repr(i2) ) )
...
(i0, i1, i2 and i3 can only be 0, 1, 2 or 3)."""
    ...
    if ( i[0] != 0 ) or ( i[1] != 1 ) or ( i[2] != 2 ) or ( i[3] != 3 ) :
        s = "Invalid i0 = %s, i1 = %s, i2 = %s and/or i3 = %s value(s)" % ( repr(i0), repr(i1), repr(i2), repr(i3) )
...
def string4dData( d, i0 = 0, i1 = 1, i2 = 2, i3 = 3, fmt0 = None, fmt1 = None, fmt2 = None, fmt3 = None ) :
    ...
    from brownies.legacy.endl import endl4dmathmisc
    i = [ i0, i1, i2, i3 ]
    i.sort( )
    if ( i[0] != 0 ) or ( i[1] != 1 ) or ( i[2] != 2 ) or ( i[3] != 3 ) :
        raise Exception( "\nError in endlmisc.string4dData: %s" % "Invalid i0 = %s, i1 = %s, i2 = %s, and/or i3 = %s" % (repr(i0),repr(i1),repr(i2),repr(i3)) )
