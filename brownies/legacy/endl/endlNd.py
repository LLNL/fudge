...
from brownies.legacy.endl import bdfls, endlmisc, endl2
...
"""
    ...
    def __init__( self, f, I_, yo, C, I, S, h, points, i0 = 0, i1 = 1, i2 = None, i3 = None, bdflsFile = None ) :
        ...
        if ( f is None ) :                              # Designer data.
            ...
            data = points
            self.designerData = 1
        elif ( hasattr(f, 'readline') ) :    # Otherwise, data must be in an endl formatted file.
            self.h = [ f.readline( ), "" ]
            if ( self.h[0] == "" ) : return
            self.h[1] = f.readline( )
        ...
        if( not( ( self.h[0] == "\n" ) and ( C == 1 ) and ( I == 0 ) ) ) :
            ...
            try :
                ...
                if( self.format < 12 ) : self.format = 12
    ...
    def compareQXs( self, Q, X1, X2, X3, X4 ) :
        ...
        if( Q is not None ) :
            if( abs( self.Q - Q ) > 1e-14 * abs( self.Q + Q ) ) :
