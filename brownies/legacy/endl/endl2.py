...
from brownies.legacy.endl import endl_y, endl_I, misc as miscModule, endl_Z, endl_C
...
def ZAToYo( ZA ) :
    ...
    raise Exception( "\nError in ZAToYo: unsupported ZA value = %s" % repr(ZA) )
...
def yoToZA( yo_i ) :
    ...
    if ( yo_i < 0 ) or ( yo_i > 16 ) : raise Exception( "\nError in yoToZA: unsupported yo value = %s" % repr(yo_i) )
    yo = yo_i
    if ( yo > 10 ) : yo -= 10
...
def residualZA_yos_Q( yi_, ZA_, C, targetELevel = 0., X4 = 0, specialCases = 1, printQWarning = True, bdflsFile = None ) :
    ...
    if ( ZAr == ZA ) :
        ...
        for yo in yos_ : endlmisc.printWarning(repr(yo))
...
def reactionEquations( yi_, ZA_, C, level = None, specialCases = 1, printQWarning = True, NameASeperator = "", bdflsFile = None ) :
    ...
    if ( ( len( yos ) > 0 ) and ( yos[0] < 0 ) ) :
        if   ( yos[0] == -1 ) :     # yo is unknown
            ...
            raise Exception( "\nError in reactionName: invalid yos[0] = %s" % repr(yos[0]) )
    else :
        n = 1
        yo_ = None
        syo = ""
...
def reactionThreshold( yi, ZA, C, targetELevel = 0., residualELevel = 0., Q = None, specialCases = 0, S = None, bdflsFile = None ) :
    ...
    if( QRequired and ( Q is None ) ) : raise Exception( 'Error from reactionThreshold: user must supply Q for requested ZA = %s or C = %d' % ( repr(ZA), C ) )
...
def symOrNameForYoOrZA( yoOrZA, wantName, ASep = "", ZAOnly = 0, AddNatural = 1, m_to_m1 = False, suffixSeperator = None ) :
    ...
    if( A == 0 ) :
        ...
        return ZStr
    return ZStr + ASep + repr(A) + Suffix
...
def reactionQByZAs( incomingZAs, outgoingZAs, bdflsFile = None ) :
    ...
    for ZA in outgoingZAs :
        ...
        ZAMultiplicities[ZA] -= 1
    mass = 0.
    ZAs = sorted( ZAMultiplicities.keys( ) )
    for ZA in ZAs :
        m = bdflsFile.mass( ZA )
