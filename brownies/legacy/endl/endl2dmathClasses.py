...
from brownies.legacy.endl import endl2dmathmisc, endlParameters, endl1dmathClasses
...
class endl2dmath :
    ...
    def __init__( self, data = None, checkDataType = False, xLabel = None, yLabel = None, label = "unknown", interpolation = 0, 
        ...
        if( ( interpolation < 0 ) or ( interpolation > 3 ) ) :
            raise Exception( "\nError in endl2dmath.__init__: Bad interpolation value = %s" % repr(interpolation) )
        ...
        while ( i1 < l1 ) and ( i2 < l2 ) :
            if ( self.data[i1][0] == other.data[i2][0] ) :
                ...
                if ( abs( d ) > f ) : print( "i1 = %5d i2 = %5d   %.5e %.5e %.5e %+.5e %+.5e" %
                                            (i1, i2, self.data[i1][0], self.data[i1][1], other.data[i2][1],
                                             self.data[i1][1] - other.data[i2][1], d) )
                i1 = i1 + 1
                i2 = i2 + 1
    ...
    def qplot( self, xMin = None, xMax = None, yMin = None, yMax = None, xylog = 0, xLabel = None, yLabel = None,
        ...
        if( ( xLabel is None ) and not( self.xLabel is None ) ) : xLabel = repr(self.xLabel)
        if( not( xLabel is None ) ) : self.g( "set xlabel %s" % repr(xLabel) )
        if( ( yLabel is None ) and not( self.yLabel is None ) ) : yLabel = repr(self.yLabel)
        if( not( yLabel is None ) ) : self.g( "set ylabel %s" % repr(yLabel) )
        if( not( title  is None ) ) : self.g( "set title %s" % repr(title) )
