...
# BRBBRB
    ...
    if endfDoc == '':
        ...
        docHeader2 = [  ' %2d-%-2s-%3d LLNL       EVAL-%3s%2s Unknown' % ( targetZ, chemicalElementMiscPoPsModule.symbolFromZ[targetZ], targetA, month, year ),
                        '                      DIST-%s                       %8s   ' % (time.strftime("%b%y").upper(), time.strftime("%Y%m%d")),
