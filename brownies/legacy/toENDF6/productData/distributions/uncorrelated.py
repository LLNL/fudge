...
# BRB - this needs to be checked.
            if targetInfo['product'].pid != IDsPoPsModule.photon:
            ...
            elif not isinstance(energySubform, regionsModule.Regions2d):
                raise NotImplementedError(f"Writing {type(energySubform)} back to MF=6")
            ...
            energyAngularSubform = energyAngularModule.XYs3d(
                axes=axes, interpolation=energySubform[0].interpolation,
                interpolationQualifier=energySubform[0].interpolationQualifier)
            EInFactor = PQUModule.PQU(1, energySubform[0].axes[2].unit).getValueAs('eV')

            # FIXME hacky way to support MF=6 LAW=1 LANG=1 with regions. Currently only seen in JEFF n+B10
            for region in energySubform[1:]:
                region[0].outerDomainValue *= (1 + 1e-9)

            for region in energySubform:
                for EIn in region:
                    if isinstance(EIn, regionsModule.Regions1d):
                        ...
                        EIn = EIn_copy
                    multiD_2d = energyAngularModule.XYs2d(outerDomainValue=EIn.outerDomainValue * EInFactor,
            ...
            for indexE, EEpP in enumerate(energySubform):
                ...
                if EEpP.outerDomainValue != EMuP.outerDomainValue:
                    raise Exception("EEpP.outerDomainValue = %s != EMuP.outerDomainValue = %s" %
                                    (EEpP.outerDomainValue, EMuP.outerDomainValue))
