...
def process_args( ) :       # see https://docs.python.org/2/howto/argparse.html
    ...
    parser.add_argument( "--skipReconstruction", default = False, action = "store_true",
                        help = "Do not reconstruct resonances" )
    ...
    parser.add_argument( "--pythonInterpreter", default = sys.executable, dest='pythonInterpreter',
...
if( 'reactionSuite' in rce ) :
    ...
    if( covariances is not None ) :
        ...
        covariances.externalFiles.add(externalFile.externalFile("reactions", RSfile))
        covariances.saveToFile( CSfile, outline = args.outline, formatVersion = args.formatVersion )

        sha1sum = checksums.sha1sum.from_file(CSfile)
        reactions.externalFiles.add(externalFile.externalFile("covariances", path=CSfile, checksum=sha1sum))
