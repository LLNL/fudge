...
this parameterization are present anymore in ENDF/B-VII.0 or later libraries. GNDS does not support Adler-Adler
...
the Single-Level Breit Wigner approximation. GNDS always stores unresolved parameters in tables, containing average partial
widths and level spacings for each energy interval, for each combination of resonance L and J.
...
'reconstructResonances' method. For example:
        ...
        >>> RS = reactionSuite.read( "gnds_formatted_file.xml" )
...
ENDF-6 manual (available at https://www.nndc.bnl.gov/csewg/docs/endf-manual.pdf).

Angular distribution reconstruction can be performed for many evaluations using ``Fudge``,
see the `examples/reconstructAngDists.py` script for a demonstration.
