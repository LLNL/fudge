...
:Usage: 
        ``<path_to_fudge>/brownies/bin/rePrint.py filename.endf``
...
After running rePrint.py, you may wish to compare the original/new ENDF files using diff, kompare, meld, etc::
...
installing extra components (``PyQt5`` and/or ``matplotlib``). A sample plotting script is included in the ``examples/``
directory::
