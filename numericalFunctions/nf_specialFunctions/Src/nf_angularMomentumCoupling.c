...
*/
    return exp( nf_amc_log_factorial( n ) );
}
/*
============================================================
*/
...
*/
    ...
    return( ( ( ( j1 - j2 - j6 ) % 4 == 0 ) ?  1.0 : -1.0 ) * cg / sqrt( j3 + 1.0 ) );          /* BRB j3 + 1 <= 0? */
}
/*
============================================================
*/
...
static double w6j1( int *x ) {
    ...
    w6j = w = exp( 0.5 * w6j + nf_amc_log_fact[k1] - nf_amc_log_fact[l1] - nf_amc_log_fact[l2] - nf_amc_log_fact[l3] - nf_amc_log_fact[l4]
...
*/
    ...
    i0 = max3( abs( j1 - j9 ), abs( j2 - j6 ), abs( j4 - j8 ) );
...
static double cg3( int x1, int x2, int x3, int y1, int y2, int y3 ) {
    ...
    a = cg = exp( 0.5 * ( nf_amc_log_fact[ x3 + y3 - 1 ] - nf_amc_log_fact[ x3 + y3 - 2 ] - nf_amc_log_fact[ nx - 1 ]
...
*/
    ...
    reduced_mat = 1.0 / sqrt( 4 * M_PI ) * clebsh_gordan / sqrt( jt + 1.0 )         /* BRB jt + 1 <= 0? */
