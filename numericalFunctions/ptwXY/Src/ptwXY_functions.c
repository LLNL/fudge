...
*/
    ...
    *status = nfu_Okay;
    ...
    if( ( n1 == 0 ) || ( n2 == 0 ) ) {
        convolute = ptwXY_new( ptwXY_interpolationLinLin, 1., accuracy, 0, 0, status, 0 );
        return( convolute );
    }
    ...
    if( n > 100000 ) mode = -1;
    if( ( convolute = ptwXY_new( ptwXY_interpolationLinLin, 1., accuracy, 400, 40, status, 0 ) ) == NULL ) return( NULL );
