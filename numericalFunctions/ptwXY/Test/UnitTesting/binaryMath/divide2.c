...
int main( int argc, char **argv ) {
    ...
    if( ( ptwXY1 = ptwXY_create( &smr, ptwXY_interpolationLinLin, NULL, biSectionMax, accuracy, 100, 10, nN, numerator, 0 ) ) == NULL )
        nfut_printSMRErrorExit2p( &smr, "Via." );
    if( ( ptwXY2 = ptwXY_create( &smr, ptwXY_interpolationLinLin, NULL, biSectionMax, accuracy, 100, 10, nD, denominator, 0 ) ) == NULL )
        nfut_printSMRErrorExit2p( &smr, "Via." );
    ...
    for( i = 0; i < division->length; i++ ) {
        ptwXY_getXYPairAtIndex( &smr, division, i, &x, &y );
