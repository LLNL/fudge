...
os.system( 'cd %s; make -s clean; ./mergeClosePoints -v > v' % CPATH )
...
def check_mergeClosePoints( ls, count ) :

    ls, epsilon = getDoubleValue( 'epsilon', ls )
    ls, data = getXYData( ls )
    ls, mergedC = getXYData( ls )
    merged = data.mergeClosePoints( epsilon )
    if( len( merged ) != len( mergedC ) ) : raise Exception( '%s: at %d len( merged ) = %d != len( mergedC ) = %d' % \
        ( __file__, count, len( merged ), len( mergedC ) ) )
    for i, xy in enumerate( merged ) :
        compareValues( "x merged", count, xy[0], mergedC[i][0] )
        compareValues( "y merged", count, xy[1], mergedC[i][1] )
    return( ls )
...
count = 0
while( len( ls ) ) :
    ls = check_mergeClosePoints( ls, count )
    count += 1
