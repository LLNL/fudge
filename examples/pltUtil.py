...
from fudge.gnd.covariances import covarianceFormToken, mixedFormToken, summedFormToken, energyIntervalFormToken #, symmetricFormToken, diagonalFormToken
...
parser.add_argument('--isGND', dest='isGND', default=False, action='store_true', help='Input file is not an ENDF formatted file, it is a GND formatted file' )
parser.add_argument('-o', dest='outFile', default=None, type=str, help='Output file for plot (disables interactive plotting)' )
parser.add_argument('--mf', dest='mf', type=int, default=3, help='MF of orginal data to plot [default is 3, cross section data, use 4 for angular, 5 for energy and 6 for energy-angle distributions and 12 for multiplicity]' )

parser.add_argument('--c4File', dest='c4File', type=str, default=None, help="Optional C4 file to pull data from instead of (or in addition to) EXFOR" )
parser.add_argument('--nox4', dest='nox4', default=False, action='store_true', help='Do not plot EXFOR data' )
parser.add_argument('--nox4evals', dest='nox4evals', default=False, action='store_true', help='Do not plot evaluations foudn in EXFOR library (with "V" SUBENT)' )
parser.add_argument('--nox4legend', dest='nox4legend', default=False, action='store_true', help='Do not put legend for EXFOR data on plot' )
parser.add_argument('--showx4legend', dest='showx4legend', default=True, action='store_true', help='Put legend for EXFOR data on plot, even if it would be automatically supressed' )

parser.add_argument('--uncratio', dest='uncratio', default=False, action='store_true', help='Plot a ratio of the uncertainty over the data' )
parser.add_argument('--angMubar', dest='angMubar', default=False, action='store_true', help='Make plot of mubar for an angular distribution' )
parser.add_argument('--angContour', dest='angContour', default=False, action='store_true', help='Make contour plot of an angular distribution' )
parser.add_argument('--Pzbar', dest='Pzbar', default=False, action='store_true', help='Make average forward scattering momentum <pz> plot from angular distribution' )
parser.add_argument('--enContour', dest='enContour', default=False, action='store_true', help='Make contour plot of an energy distribution' )
parser.add_argument('--Ebar', dest='Ebar', default=False, action='store_true', help="Make average emitted/deposited energy <E'> plot of an energy distribution" )
parser.add_argument('--nounc', dest='nounc', default=False, action='store_true', help='Do not plot uncertainties' )
parser.add_argument('--simplelegend', dest='simplelegend', default=False, action='store_true', help='Do not put entries for the components of requested MT in the plot legend' )
parser.add_argument('--showparts', dest='showparts', default=False, action='store_true', help='Show all of the channels that comprise the one requested (if applicable)' )

parser.add_argument('--logx', dest='logx', default=None, action='store_true', help='Make x scale logrithmic (Default is False except for MT in [1,2,18,102])' )
parser.add_argument('--logy', dest='logy', default=None, action='store_true', help='Make y scale logrithmic (Default is False except for MT in [1,2,18,102])' )
parser.add_argument('--logz', dest='logz', default=None, action='store_true', help='Make z scale logrithmic' )

parser.add_argument('--xUnit', dest='xUnit', default=None, type=str, help='Use units of xUnit for the x axis (Defaults to eV for MF=3)' )
#parser.add_argument('--xMin', dest='xMin', default=None, type=float, help='Min. x in plots, in units of xUnit.  Setting this turns off autoscaling for xmin.' )
#parser.add_argument('--xMax', dest='xMax', default=None, type=float, help='Max. x in plots, in units of xUnit.  Setting this turns off autoscaling for xmax.' )

parser.add_argument('--yUnit', dest='yUnit', default=None, type=str, help='Use units of yUnit for the y axis (Defaults to b for MF=3)' )
#parser.add_argument('--yMin', dest='yMin', default=None, type=float, help='Min. y in plots, in units of yUnit.  Setting this turns off autoscaling for ymin.' )
#parser.add_argument('--yMax', dest='yMax', default=None, type=float, help='Max. y in plots, in units of yUnit.  Setting this turns off autoscaling for ymax.' )

parser.add_argument('--zUnit', dest='zUnit', default=None, type=str, help='Use units of zUnit for the z axis' )
#parser.add_argument('--zMin', dest='zMin', default=None, type=float, help='Min. z in plots, in units of zUnit.  Setting this turns off autoscaling for zmin.' )
#parser.add_argument('--zMax', dest='zMax', default=None, type=float, help='Min. z in plots, in units of zUnit.  Setting this turns off autoscaling for zmax.' )
...
parser.add_argument('--target', dest='target', default=None, type=str, help="The target nucleus, given in GND notation, e.g. Pu239 (Default is None which means to take it from the first ENDF file)" )
parser.add_argument('--projectile', dest='projectile', default=None, type=str, help="The projectile particles, given in GND notation, e.g. n (Default is None which means to take it from the first ENDF file)" )
parser.add_argument('--product', dest='product', default=None, type=str, help="The product particle of interest, given in GND notation, e.g. n (Default is None which means to take the first emitted particle for this MT)" )
...
def getUncertainty( theCovariance, theData ):
    ...
    theUncert = None
    for form in ( covarianceFormToken, mixedFormToken ):
...
def getEvaluationMTs( reactionSuite, mtFilter = None ):
    if mtFilter == None: return [ int( r.attributes['ENDF_MT' ] ) for r in reactionSuite.reactions ]
    else:                return [ int( r.attributes['ENDF_MT' ] ) for r in reactionSuite.reactions if int( r.attributes['ENDF_MT' ] ) in mtFilter ]


def getReactions( reactionSuite, MT ): 
    return [ r for r in reactionSuite.reactions + reactionSuite.summedReactions if r.attributes['ENDF_MT' ] == str(MT) ]
...
def getPointwiseCrossSection( reac ):
    ...
    elif 'piecewise'         in reac.crossSection.forms: return reac.crossSection['piecewise'].toPointwiseLinear( 0, 1e-8 )
    elif 'weightedPointwise' in reac.crossSection.forms: return reac.crossSection['weightedPointwise'].toPointwiseLinear( 0, 1e-8 )
    elif 'resonancesWithBackground' in reac.crossSection.forms: # in case --noReconstruct option is used:
        return reac.crossSection['resonancesWithBackground'].tabulatedData.toPointwiseLinear( 0, 1e-8 ) 
    return None
...
def getEXFORSets( sym, A, reaction = None, quantity = "SIG", nox4evals = True, nox4legend = False, forceLegend = False ): 
    ...
    suppressEXFORLegend = not forceLegend and ( nox4legend or len( subents.keys() ) > 20 )
...
# cross section plots
#---------------------------------------------------
def makeCSPlot( gndMap, mt, projectile, target, uncratio=False, nounc=False, nox4=False, showparts=False, nox4evals=True, nox4legend = False, xLog=None, xUnit='eV', yLog=None, yUnit='b', outFile=args.outFile ):
    ...
    if endfData + exforData +c4Data != []:
        ...
        plot2d.makePlot2d( exforData + c4Data + endfData, xAxisSettings = xAxisSettings, 
            yAxisSettings = yAxisSettings, title = target.capitalize() + '(' + projectile+','+reactionString +')', legendOn = True, outFile = outFile )

#---------------------------------------------------
# multiplicity plots
#---------------------------------------------------
def makeMultiplicityPlot( gndMap, mt, projectile, target, product, xLog=None, xUnit=args.xUnit, yLog=None, yUnit=args.yUnit, yFrame='centerOfMass', zLog=None, zUnit='b', outFile=args.outFile ): 
...
#---------------------------------------------------
def makeEnDistSlicesPlot( gndMap, mt, projectile, target, product, xLog=None, xUnit=args.xUnit, yLog=None, yUnit=args.yUnit, yFrame='centerOfMass', zLog=None, zUnit='b', outFile=args.outFile ): 

    raise NotImplementedError()

def makeEnDistContourPlot( gndMap, mt, projectile, target, product, xLog=None, xUnit=args.xUnit, yLog=None, yUnit=args.yUnit, yFrame='centerOfMass', zLog=None, zUnit='b', outFile=args.outFile ): 
...
#---------------------------------------------------
def makeEnDepPlot( gndMap, mt, projectile, target, product, xLog=None, xUnit=args.xUnit, yLog=None, yUnit=args.yUnit, yFrame='centerOfMass', zLog=None, zUnit='b', outFile=args.outFile ): 

    raise NotImplementedError()

def makeMomDepPlot( gndMap, mt, projectile, target, product, xLog=None, xUnit=args.xUnit, yLog=None, yUnit=args.yUnit, yFrame='centerOfMass', zLog=None, zUnit='b', outFile=args.outFile ): 
...
#---------------------------------------------------
def makeAngDistContourPlot( gndMap, mt, projectile, target, product, xLog=None, xUnit=args.xUnit, yLog=None, yUnit=args.yUnit, yFrame='centerOfMass', zLog=None, zUnit='b', outFile=args.outFile ): 
    ...
    for endf in gndMap:
        ...
        for r in reactionList:
            ...
            for a in angDistData:
                endfData.append( a.toPointwiseLinear( 1e-6 ) )
    ...
    if endfData != []:
        ...
        plot2d.makePlot2dContour( endfData, xAxisSettings = xAxisSettings, yAxisSettings = yAxisSettings, 
            title = "$d^2\\sigma(E)/dE'd{\\mu}$ for emitted "+product+"'s from "+target.capitalize() + '(' + projectile+', '+reactionString +')', legendOn = False, outFile = outFile )
            
def makeAngDistSlicesPlot( gndMap, mt, projectile, target, product, xLog=None, xUnit=args.xUnit, yLog=None, yUnit=args.yUnit, yFrame='centerOfMass', zLog=None, zUnit='b', outFile=args.outFile ): 

    raise NotImplementedError()

def makeAngDistMubarPlot( gndMap, mt, projectile, target, product, xLog=None, xUnit=args.xUnit, yLog=None, yUnit=args.yUnit, yFrame='centerOfMass', outFile=args.outFile ): 
    ...
    for endf in gndMap:
        ...
        for r in reactionList:
            ...
            for a in angDistData:
                xUnit = a.axes[0].unit
                if yFrame != a.axes[1].frame: raise ValueError( "Found frame ",a.axes[1].frame, "you requested frame", yFrame )
    ...
    if endfData != []:
        ...
        plot2d.makePlot2d( endfData, xAxisSettings = xAxisSettings, yAxisSettings = yAxisSettings, 
            title = "$<\mu_{"+yFrameStr+"}>$ for emitted "+product+"'s from "+target.capitalize() + '(' + projectile+', '+reactionString +')', legendOn = True, legendXY=(0.50,0.2), outFile = outFile )
...
if __name__ == "__main__": 
    ...
    for endf in args.endf:
        ...
        gndMap[endf] = endfFileToGND.endfFileToGND( endf, toStdOut = False, skipBadData = True )
        print
    ...
    if args.mf == 3:        makeCSPlot( gndMap, mt=args.mt[0], projectile=projectile, target=target, uncratio=args.uncratio, nounc=args.nounc, nox4=args.nox4, showparts=args.showparts, nox4evals=args.nox4evals, nox4legend=args.nox4legend, xLog=args.logx, xUnit=args.xUnit, yLog=args.logy, yUnit=args.yUnit, outFile=args.outFile )
    elif args.mf == 4: 
        if args.angContour: makeAngDistContourPlot( gndMap, mt=args.mt[0], projectile=projectile, target=target, product=product, xLog=args.logx, xUnit=args.xUnit, yLog=args.logy, yUnit=args.yUnit, zLog=args.logz, zUnit=args.zUnit, outFile=args.outFile )
        elif args.angMubar: makeAngDistMubarPlot( gndMap, mt=args.mt[0], projectile=projectile, target=target, product=product, xLog=args.logx, xUnit=args.xUnit, yLog=args.logy, yUnit=args.yUnit, outFile=args.outFile )
        elif args.Pzbar:    makeMomDepPlot( gndMap, mt=args.mt[0], projectile=projectile, target=target, product=product, xLog=args.logx, xUnit=args.xUnit, yLog=args.logy, yUnit=args.yUnit, outFile=args.outFile )
        else:               makeAngDistSlicesPlot( gndMap, mt=args.mt[0], projectile=projectile, target=target, product=product, xLog=args.logx, xUnit=args.xUnit, yLog=args.logy, yUnit=args.yUnit, zLog=args.logz, zUnit=args.zUnit, outFile=args.outFile )
    elif args.mf == 5: 
        if args.enContour:  makeEnDistContourPlot( gndMap, mt=args.mt[0], projectile=projectile, target=target, product=product, xLog=args.logx, xUnit=args.xUnit, yLog=args.logy, yUnit=args.yUnit, zLog=args.logz, zUnit=args.zUnit, outFile=args.outFile )
        elif args.Ebar:     makeEnDepPlot( gndMap, mt=args.mt[0], projectile=projectile, target=target, product=product, xLog=args.logx, xUnit=args.xUnit, yLog=args.logy, yUnit=args.yUnit, outFile=args.outFile )
        else:               makeEnDistSlicesPlot( gndMap, mt=args.mt[0], projectile=projectile, target=target, product=product, xLog=args.logx, xUnit=args.xUnit, yLog=args.logy, yUnit=args.yUnit, zLog=args.logz, zUnit=args.zUnit, outFile=args.outFile )
    else:                   raise NotImplementedError( "Only MF=3 (cross section data), MF=4 (angular distributions) & MF=5 (energy distributions)  currently implemented, you requested MF="+str(args.mf) )
