...
Interpolation values and meaning::
   -----------------------
    ...
    def integrateOneFunction( self, xMin = None, xMax = None ) :
        ...
        integral = fudge2dGrouping.groupOneFunction( [ xMin, xMax ], self )
        return( integral[0] )
    ...
    def integrateTwoFunctions( self, other, xMin = None, xMax = None ) :
        ...
        integral = fudge2dGrouping.groupTwoFunctions( [ xMin, xMax ], self, other )
        return( integral[0] )
    ...
    def integrateThreeFunctions( self, other1, other2, xMin = None, xMax = None ) :
        ...
        integral = fudge2dGrouping.groupThreeFunctions( [ xMin, xMax ], self, other1, other2 )
        return( integral[0] )
    ...
    ( 1. + parameters.endlEpsx ) is inserted  with y value of self at that x value."""
        ...
        if ( len( other ) == 0 ) :
            ...
            i = 0
            n = len( self.data )
            xMin = other.data[0][0] * ( 1. - parameters.endlEpsx )
            ...
            xMax = other.data[-1][0] * ( 1. + parameters.endlEpsx )
            while ( i < n ) :
                xy = self.data[i]
                if ( xy[0] > xMax ) : break
                i += 1
