...
def check2dData( data, allowNegativeX = True, allowSameX = False, allowZeroX = True, positiveY = True, printWarning = True, printErrors = True, 
    ...
    if( xCloseEps == None ) :
        ...
        xCloseEps = parameters.endlEpsx / 3. * pow( 0.1, ( formatCode - 14 ) )
    if( l > 0 ) :
        pl = None
        pn = points[0]
        i = 0
...
def dullLowerEdge2d( d ) :
    ...
    x = d.data[0][0]
    if ( x > parameters.endlSmallx ) :
        if ( len( d.data ) > 1 ) and ( d.data[1][0] > ( 1. + 2. * parameters.endlEpsx ) * x ) :
            d.data[0][1] = d[( 1. + parameters.endlEpsx ) * x]
            d.data[0][0] = ( 1. + parameters.endlEpsx ) * x
        d.data.insert( 0, [ ( 1 - parameters.endlEpsx ) * x, 0. ] )
    return d 
...
def dullUpperEdge2d( d ) :
    ...
    x = d.data[-1][0]
    if ( x < parameters.endlLargex ) :
        if ( len( d.data ) > 1 ) and ( d.data[-2][0] < ( 1. - 2. * parameters.endlEpsx ) * x ) :
            d.data[-1][1] = d[( 1. - parameters.endlEpsx ) * x]
            d.data[-1][0] = ( 1. - parameters.endlEpsx ) * x
        d.data.append( [ ( 1 + parameters.endlEpsx ) * x, 0. ] )
    return d
...
def convertFunctionToLinLin( f, xMin, xMax, accuracy ):
    ...
    result = endl2dmathClasses.endl2dmath( data = [ [ xMin, f( xMin ) ], [ xMax, f( xMax ) ] ] )
    midPt = ( xMin + xMax )/2.0
    while abs( result.getValue( midPt ) - f( midPt ) ) > accuracy :
        result = endl2dmathClasses.endl2dmath( data = convertFunctionToLinLin( f, xMin, midPt, accuracy ).data + convertFunctionToLinLin( f, midPt, xMax, accuracy ).data )
    return result
