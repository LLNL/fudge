...
"""
    ...
    def fixQandThreshold( self, Q = None, specialCases = 0, fixThresholdMode = None, dThreshold_MeV = 10e-3, EMin = 0., threshold_MeV_shiftWarning = 0.1 ) :

        if( self.C == 6 ) : return( 0., 0. )
        if( Q == None ) :
        ...
        if( ( EMin <= threshold <= 0.0 ) or ( len( self ) == 0 ) ) : return( Q, 0. )
        if( hasattr( self, 'fixThreshold' ) ) : self.fixThreshold( threshold = threshold, dThreshold_MeV = dThreshold_MeV, EMin = EMin,
