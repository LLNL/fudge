...
class Test_summed(TestCaseBase):
    ...
    def setUp(self):
        ...
        bds = abc.rowBounds()
        self.sumabc = covariances.summed.SummedCovariance(label='test', domainMin=float(bds[0]),
                                                          domainMax=float(bds[1]),
                                                          domainUnit=abc[0].matrix.axes[-1].unit, summands=[
                covariances.summed.Summand(link=abc, path=abc.toXLink(), coefficient=1.0)])

    @unittest.skip("FIXME")
    def test__getitem__(self):
        self.assertXMLListsEqual(self.sumabc.summands[0].toXML_strList(),
                                 ['<link coefficient="1.0" xlink:href="/mixed"/>'])
